...
bool memoryCache:: add(string &key, string &value)
{
    ...
    int requiredNode = value_size/N + 1;// for end point
    cout<<" require Node : "<<requiredNode<<" \n";
    // if(value_size%N == 0) // for end point
    // {
    //     requiredNode++;
    // }
    new_value = NULL;
    ...
    while(true)
    {
        while(node_index<N && value_index<value_size) // N  buffer size in node
        {
        ...
        if(node_index<N && value_index==value_size)
        {
            break;
        }

        // it should be after if statment otherwise it will give segmentation in line curr->value
        curr = curr->next; // this is always works as we assign a extra node when value_size%N == 0 
        node_index = 0;
    }
...
bool memoryCache::update(string &key,string &value)
{
...
bool memoryCache::read(string &key,char *buffer)
{
    ...
    while(true)
    {
        while(node_index < N && node->value[node_index]!='\0')
        {
            buffer[i++] = node->value[node_index++];
        }
        if(node_index < N && node->value[node_index]=='\0')
        {
            buffer[i++] = '\0';
            break;
        }
